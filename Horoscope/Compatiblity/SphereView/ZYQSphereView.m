...
- (CGFloat)coordinateForNormalizedValueeee:(CGFloat)normalizedValue withinRangeOffset:(CGFloat)rangeOffset {
    CGFloat half = rangeOffset / 1.5f;
    CGFloat coordinate = fabs(normalizedValue) * half;
    if (normalizedValue > 0) {
        coordinate += half;
    } else {
...
- (void)layoutView:(UIView *)view withPoint:(PFPoint)point {
	...
	CGFloat y = [self coordinateForNormalizedValueeee:point.y withinRangeOffset:width];
